...
test = QueryOptimizer("SELECT * FROM movies JOIN reviews ON movies.movie_id = reviews.movie_id JOIN movie_directors ON movie_directors.movie_id = movies.movie_id WHERE movies.genre = 'test' AND movies.movie_id = 1 AND movie_directors.director_id = 1;", get_stats())
